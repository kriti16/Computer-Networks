...
int main(int argc, char *argv[]){
    ...
    char welcomeMessage[SMALL] = "Welcome :)\nEnter file name:";
    char filename[SMALL];
   
    while(true){
        //Wait for client
        int clientFileDescriptor = accept(socketFileDescriptor,NULL,NULL);
        FILE* clientWrite = fdopen(clientFileDescriptor,"w");

        // Send welcome message
        int sentData = send(clientFileDescriptor,welcomeMessage,SMALL,0);
        while(sentData < SMALL){
            cout<<"\n\nOnly "<<sentData<<" bytes were sent instead of "<<SMALL<<" bytes\n\n";
            sentData += send(clientFileDescriptor,welcomeMessage+sentData,SMALL-sentData,0);
        }
        
        while(true){   
            int recData = recv(clientFileDescriptor,filename,SMALL,0);
            if(recData == 0){
                printf("Waiting for client ....\n");
                break;
            }
            while(recData < SMALL){
                recData += recv(clientFileDescriptor,filename+recData,SMALL-recData,0);
            }     
            cout<<"Required File : "<<filename<<endl;
