...
def compare_url(argtuple):
     ...
     for (proxy, direct) in zip(proxy_data, direct_data):
          if proxy != direct and not (proxy.startswith('Date') and direct.startswith('Date')) and not (proxy.startswith('Expires') and direct.startswith('Expires'))  and not (proxy.startswith('Age') and direct.startswith('Age')):
               print 'Proxy: %s' % proxy
